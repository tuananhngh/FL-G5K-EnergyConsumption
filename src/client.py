...
class Client(fl.client.NumPyClient):
    ...
    def write_time_csv(self, path, client_id, round, call, status):
        with open(path, "a", newline="") as f:
            ...
            writer.writerow(
                [client_id, round, now.strftime("%Y-%m-%d %H:%M:%S.%f"), call, status]
            )

    def fit(
        self, parameters, config: Dict[str, Scalar] = None
    ...
    def evaluate(
        ...
        path_comm = Path(self.outputdir, f"client_{self.cid}_comm.csv")
        self.write_time_csv(
            path_comm, self.cid, config["server_round"], "evaluate", "start"
        )
        server_round = config["server_round"]
        self.set_parameters(parameters)
        loss, accuracy = test(self.model, self.valloader, self.device, verbose=True)
        ...
        with open(path, "a", newline="") as f:
            ...
            now = datetime.datetime.now()
            writer.writerow(
                [now.strftime("%Y-%m-%d %H:%M:%S.%f"), server_round, loss, accuracy]
            )
        self.write_time_csv(
            path_comm, self.cid, config["server_round"], "evaluate", "end"
        )
        return float(loss), len(self.valloader.dataset), {"accuracy": accuracy}

    def client_dry_run(
        self, model, client_id, trainloaders, valloaders, config, device
    ):
        local_epochs = config["local_epochs"]
        lr = config["lr"]
        ...
        result = train(
            model, self.trainloader, self.valloader, local_epochs, optim, self.device
        )
        return result
...
def main(cfg: DictConfig):
    ...
    if dry_run:
        res = client.client_dry_run(
            model, client_id, trainloader, valloader, cfg.client_params, device
        )
        logging.info(res)
    else:
