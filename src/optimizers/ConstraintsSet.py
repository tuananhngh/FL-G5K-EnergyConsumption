...
# Code taken and modify from https://github.com/ZIB-IOL/StochasticFrankWolfe
...
def get_avg_init_norm(layer, param_type=None, ord=2, repetitions=100):
    ...
    for _ in range(repetitions):
        ...
        output += torch.norm(getattr(layer, param_type), p=ord).item() #type:ignore
...
@torch.no_grad()
def make_feasible_origin(model, constraints):
    """Shift all model parameters to 0"""
    for idx, (name, param) in enumerate(model.named_parameters()):
        constraint = constraints[idx]
...
class LpBall(Constraint):
    ...
    def __init__(self, n, ord=2, diameter=None, radius=None):
        ...
        if diameter is None and radius is None:
            ...
            self._radius = radius
            self._diameter = 2 * convert_lp_radius(radius, self.n, in_ord=self.p, out_ord=2) #type:ignore
        elif radius is None:
            self._radius = convert_lp_radius(diameter / 2.0, self.n, in_ord=2, out_ord=self.p) #type:ignore
    ...
    def lmo(self, x):
        ...
        if self.p == 1:
            ...
            v.view(-1)[maxIdx] = -self._radius * torch.sign(x.view(-1)[maxIdx]) #type:ignore
            return v
        elif self.p == 2:
            x_norm = float(torch.norm(x, p=2))
            if x_norm > tolerance:
                return -self._radius * x.div(x_norm) #type:ignore
            else:
                return torch.zeros_like(x)
        elif self.p == float('inf'):
            return torch.full_like(x, fill_value=self._radius).masked_fill_(x > 0, -self._radius) #type:ignore
        else:
            ...
            if x_norm > tolerance:
                return -self._radius / x_norm * sgn_x * absxqp #type:ignore
            else:
                return torch.zeros_like(x)
    ...
    def euclidean_project(self, x):
        ...
        if self.p == 1:
            ...
            return torch.clamp(x, min=-self._radius, max=self._radius) #type:ignore
        else:
...
class KSparsePolytope(Constraint):
    ...
    def __init__(self, n, K=1, diameter=None, radius=None):
        ...
        if diameter is None and radius is None:
            ...
            self._radius = radius
            self._diameter = 2.0 * radius * math.sqrt(self.k) #type:ignore
        elif radius is None:
    ...
    def lmo(self, x):
        ...
        v.view(-1)[maxIndices] = -self._radius * torch.sign(x.view(-1)[maxIndices]) #type:ignore
        return v
