...
class FedSFW(FedAvg):
    ...
    def aggregate_fit(
        ...
        # We log the sparsity of the weights
        sparsity_dict = {}
        #Check sparsity of the weights
        for i,(weights, _) in enumerate(weights_results[:1]):
            for j,weight in enumerate(weights):
                if len(weight.shape) > 1:
                    sparsity = np.count_nonzero(weight)/weight.size
                    sparsity_dict[f'client_{i}_layer_{j}'] = sparsity

        #print("Sparsity : ",sparsity_dict)
        with open(os.path.join(self.info_path,'sparsity.log'), 'a') as f: #type: ignore
            f.write(f"Sparsity {server_round} : "+str(sparsity_dict) + "\n")

        aggregate_params = aggregate(weights_results)
        # sparse_aggregate_params:NDArrays = []
        # for agg_params,init_params in zip(aggregate_params, self.init_params): 
        #     diff = np.maximum(np.abs(agg_params) - np.abs(init_params), 0)
        #     #logging.info("---------------------------------------------")
        #     #logging.info(f"Weight Shape :{diff.shape} {agg_params.shape}")
        #     #logging.info(f"Weight Size :{diff.size} {agg_params.size}")
        #     #logging.info(f"Weight Sparse :{np.count_nonzero(diff)/diff.size} {np.count_nonzero(agg_params)/agg_params.size}")
        #     sparse_aggregate_params.append(diff)
